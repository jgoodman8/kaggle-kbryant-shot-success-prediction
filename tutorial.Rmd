...
```{r}
train$visitor <- ifelse(grepl("@", train$matchup), "HOME", "AWAY")
...
Para utilizar repeats ---> usar repeatedcv como method. (cv no funciona con repeat)

```{r}
control <- caret::trainControl(method = "repeatedcv",repeats = 3, classProbs = TRUE)
```



```{r}
modelXGB <- caret::train(shot_made_flag ~ ., data = train.partition, trControl = control, method = "pls", metric = "logLoss", preProc = c("center"), 
                         tuneLength = 20)
modelXGB
```
...
prediction <- predict(modelXGB, newData = validation.partition.test, type = "prob")
```

```{r}
resamples.result <- resamples(list(xgb = modelXGB, adaboost = modelAdaboost))
