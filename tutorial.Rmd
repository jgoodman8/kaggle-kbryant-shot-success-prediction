...
Para realizar tal partición, se puede valorar el uso de la función *createFolds* con *k = 2*, obteniendo dos subconjuntos balanceados pero de dimensiones casi identicas. Por otra parte, la función *createResample* crea las particiones utilizando *bootstraping*, con lo que podríamos tener instancias presentes en ambos subconjuntos. Nos interesa crear dos subconjuntos balanceados y de unas proporciones de 70% para el subconjunto de entrenamiento y 30% para el de test, con respecto al conjunto original. Para ello, utilizamos *createDataPartition* con el parámetro *p = .7*.
...
Uno de los métodos elegidos para general nuestro modelo de clasificación es *glmnet* (*Lasso and Elastic-Net Regularized Generalized Linear Models*). Este algoritmo utiliza descenso cíclico coordinado, el cual optimiza sucesivamente la función objetivo sobre cada parémetro con otros fijos, hasta llegar a coverger. Utiliza los parámetros *alpha* y *lambda*. El primero es el modifica el valor de mezcla de la regulzación *elastic net*, variando entre 1 (lasso) y 0 (ridge). El segundo parámetro *lambda* se calcula en función de del valor de *alpha* y el número de valores de la secuencia (por defecto 100).

```{r}
glmnet.info <- caret::getModelInfo("glmnet")
glmnet.info$fda$parameters
```

A continuación, en el modelo *modelGLM.default* realizamos un entrenamiento utilizando 3^2 combinaciones de dichos parámetros. En el modelo *modelGLM.custom* amplicamos el parámetro *tuneLength = 20*, por lo que combinaremos 20 valores de cada uno de los parámetros, obteniedo un total de 20^2 combinaciones.
...
Dado que el ambos modelos (*fda* y *glmnet*) han sido entrenados con las mismas hojas (no se ha modificado la semilla), podemos comparar los resultados de la crossvalidación de ambos modelos.

```{r}
resamples.result <- caret::resamples(list(glm = modelGLM, fda = modelFDA.custom))
...
Antes de generar una clasificación sobre el conjunto de test, necesitamos eliminar y transformar las variables tal y como lo hicimos en el conjunto de entrenamiento. El modelo ha sido entrenado para "comprender" unas determinadas variables de tereminados tipos, por lo que es necesario aplicar las mismas operaciones.
...
for (i in 1:ncol(test)) {
    if (is.factor(test[, i])) {
        levels(test[, i]) <- c(1:length(levels(test[, i])))
        test[, i] <- as.numeric(test[, i])
    }
}
```

Finalmente, generamos la clasificación sobre el conjunto de test que nos ha proporcionado *Kaggle*. 

```{r}
prediction <- predict(model, newdata = test, type = "prob")
prediction.table <- data.frame(shot_id = shot_id, shot_made_flag = prediction$ERROR)
